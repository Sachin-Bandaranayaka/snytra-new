...
interface OnlineOrderingEditorProps {
    initialContent: Partial<OnlineOrderingData>;
    onChange: (newContent: Partial<OnlineOrderingData>) => void;
}

export default function OnlineOrderingEditor({ initialContent, onChange }: OnlineOrderingEditorProps) {
    const [content, setContent] = useState<Partial<OnlineOrderingData>>(initialContent);

    useEffect(() => {
        setContent(initialContent);
    }, [initialContent]);

    const handleUpdate = (newContent: Partial<OnlineOrderingData>) => {
    ...
    const handleArrayChange = (arrayPath: string[], index: number, field: string, value: string) => {
        const newContent = JSON.parse(JSON.stringify(content));
        let currentArray: any[] = arrayPath.reduce((acc, key) => acc[key] as any[], newContent);
