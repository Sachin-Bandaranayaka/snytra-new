        ...
        const fetchPackageData = async () => {
            try {
                ...
                if (data.success && data.plan) {
                    ...
                    // Extract features and map to IDs when possible
                    let features = Array.isArray(plan.features) ? plan.features : [];

                    // Try to match feature names with our system features
                    const featureIds = features.map(feature => {
                        // Try to find a matching system feature by name
                        const matchingFeature = SYSTEM_FEATURES.find(
                            sf => sf.name.toLowerCase() === feature.toLowerCase()
                        );

                        return matchingFeature ? matchingFeature.id : feature;
                    });
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                {getFeaturesByCategory(category).map(feature => (
                                                    <div key={feature.id} className="flex items-start">
                                                        ...
                                                        <label htmlFor={`edit-${feature.id}`} className="ml-2 block text-sm text-gray-700 cursor-pointer">
